...
window.addEventListener('load', function () {
    ...
    let auto_close_liveroom_without_red_packet = true
    // 是否启用自动删除无用内容 false是禁用 true是启用
    let auto_remove_liveroom_useless = true
    // 循环判断直播间有无红包时间间隔 默认3分钟
    let is_liveroom_have_red_packet_loop_time = 3 * 60 * 1000
    ...
    if(window.location.href.startsWith("https://live.bilibili.com/p/eden/area-tags")) {
        ...
        try {
            ...
            console.log("loop_run_time=" + loop_run_time)
        } catch {
    ...
    // 位于直播页面
        ...
        if(auto_remove_liveroom_useless) {
            // 5秒后自动删除无用内容
            setTimeout(() => {
                remove_useless()
            }, 5000);
        ...
        if(auto_close_liveroom_without_red_packet) {
            setInterval(() => {
            ...
            }, is_liveroom_have_red_packet_loop_time);
        }   
    }
})